...
Arşiv dosya ve klasör adları İngilizce `snake_case` biçimine dönüştürüldü; kullanıcının seçtiği güncel CV adı `enesdemirag-resume.pdf` olarak korundu. `manifest.json` içindeki `sources.path` alanları tarihsel Git yollarını aynen korur. Sonradan eklenen dosyalar manifestte `local_asset` olarak işaretlidir.
